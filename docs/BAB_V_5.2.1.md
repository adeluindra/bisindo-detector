...
Tahap awal sebelum model klasifikasi diuji dan ditayangkan pada peramban web adalah *preprocessing* dataset secara *offline*. Tahap ini bertujuan untuk mentransformasikan sekumpulan berkas citra mentah gerakan tangan menjadi bentuk vektor fitur numerik terstruktur yang siap dibaca oleh algoritma klasifikasi *K-Nearest Neighbor* (KNN). 

Pemrosesan ini diimplementasikan menggunakan skrip Python 3.10 dengan memanfaatkan pustaka OpenCV (untuk manajemen berkas gambar dan manipulasi warna citra) serta framework kecerdasan buatan MediaPipe Hands (untuk melacak dan mengekstrak koordinat landmark 3D dari tangan).
...
Untuk keperluan evaluasi performa model klasifikasi secara objektif, seluruh kumpulan citra yang ada di dalam dataset harus dibagi menjadi data latih (*training*) dan data uji (*testing*). Pada sistem ini, pembagian data dilakukan secara otomatis menggunakan metode pembagian berstrata (*stratified split*) dengan proporsi **80% untuk data training** dan **20% untuk data testing**.

Pembagian secara berstrata (*stratified*) sangat penting untuk memastikan keseimbangan distribusi kelas. Dengan metode ini, setiap kelas huruf dari A sampai Z secara individual akan diambil tepat 80% dari total gambarnya untuk dilatih, dan 20% sisanya disimpan khusus sebagai data uji. Hal ini mencegah terjadinya ketidakseimbangan kelas (*class imbalance*) yang dapat menurunkan akurasi model untuk huruf-huruf tertentu.

#### C. Deteksi Landmark Menggunakan MediaPipe Hands
MediaPipe Hands digunakan sebagai ekstraktor fitur utama. Dalam fase offline preprocessing, MediaPipe Hands dikonfigurasi menggunakan mode gambar diam dengan parameter `static_image_mode=True`. Pengaturan ini memaksa model deteksi untuk memperlakukan setiap citra gambar sebagai file independen (tidak saling berhubungan) dan bukan sebagai aliran video berurutan, sehingga menghasilkan akurasi koordinat landmark yang lebih presisi pada citra foto diam.

Saat citra dibaca oleh program, alur pemrosesannya adalah sebagai berikut:
1. Citra dimuat dari disk menggunakan fungsi `cv2.imread(path)` yang menghasilkan representasi matriks piksel dalam format warna BGR (*Blue-Green-Red*).
2. Citra BGR dikonversi menjadi format RGB (*Red-Green-Blue*) dengan fungsi `cv2.cvtColor(image, cv2.COLOR_BGR2RGB)`. Langkah konversi warna ini bersifat wajib karena model deteksi MediaPipe Hands dilatih menggunakan gambar berskema warna RGB.
3. Citra RGB diproses menggunakan fungsi `hands.process(image_rgb)` untuk melacak keberadaan tangan dan mengekstrak koordinat geometris 21 titik kunci tangan (*hand landmarks*).

Setiap titik kunci tangan yang terdeteksi memiliki koordinat 3 dimensi $P_i = (x_i, y_i, z_i)$ yang nilainya dinormalisasi secara bawaan oleh MediaPipe relatif terhadap dimensi lebar dan tinggi gambar.
...
Untuk mengatasi kendala ini, program preprocessing melakukan normalisasi koordinat secara matematis:
1.  **Translasi Relatif**: Pergelangan tangan (*wrist* indeks ke-0, $P_0$) ditetapkan sebagai titik pusat koordinat $(0,0,0)$. Seluruh koordinat landmark lainnya ($P_1$ sampai $P_{20}$) dikurangi dengan koordinat pergelangan tangan tersebut untuk menghasilkan koordinat relatif:
    $$P'_i = P_i - P_0 = (x_i - x_0, y_i - y_0, z_i - z_0)$$
2.  **Penskalaan Skala**: Sistem mencari nilai absolut koordinat relatif terbesar ($V_{\max}$) di seluruh sumbu $x, y, z$:
    $$V_{\max} = \max_{i, d \in \{x, y, z\}} |P'_{i,d}|$$
3.  **Pembagian Skala**: Seluruh koordinat relatif dibagi dengan $V_{\max}$ untuk menyamakan ukuran tangan sehingga skalanya berada dalam rentang $[-1.0, 1.0]$:
    $$P''_i = \frac{P'_i}{V_{\max}}$$

Melalui normalisasi ini, sistem akan menghasilkan nilai fitur yang konsisten meskipun tangan berada dekat atau jauh dari kamera.

#### E. Perhitungan Sudut Sendi Jari (*Joint Angle*)
Untuk memperkaya fitur klasifikasi selain koordinat spasial, dihitung pula sudut derajat tekukan dari persendian jari tangan (*joint angles*). Fitur sudut sendi sangat efektif untuk membedakan bentuk isyarat jari karena sifatnya yang kokoh terhadap rotasi tangan (*rotation-invariant*).
...
Untuk itu, program merancang penggabungan fitur dengan dimensi statis berukuran **156 fitur**:
*   Fitur Tangan Kiri: Terdiri dari 63 nilai koordinat normalisasi (21 titik $\times$ 3 sumbu) ditambah 15 nilai sudut sendi jari (Total 78 fitur).
*   Fitur Tangan Kanan: Terdiri dari 63 nilai koordinat normalisasi ditambah 15 nilai sudut sendi jari (Total 78 fitur).

Vektor fitur akhir dibentuk dengan menggabungkan larik tangan kiri dan kanan secara berurutan (`[fitur_kiri, fitur_kanan]`). Apabila salah satu tangan tidak terdeteksi dalam gambar (misalnya isyarat huruf 'A' yang hanya memakai tangan kanan), maka slot fitur untuk tangan kiri yang kosong akan diisi dengan larik penyangga berisi nilai $0.0$ sebanyak 78 elemen. Mekanisme ini menjamin input klasifikasi selalu memiliki panjang tepat 156 dimensi.
...
Implementasi lengkap skrip Python untuk preprocessing dataset offline ini disajikan pada Kode Program 5.22.

```python
# Kode Program 5.22 Preprocessing Dataset Offline (scripts/preprocess.py)
import cv2
...
# Inisialisasi pustaka MediaPipe Hands untuk gambar statis
...
# Indeks persendian jari untuk perhitungan sudut sendi
FINGER_JOINTS = {
    "thumb":  [0, 1, 2, 3, 4],
    "index":  [0, 5, 6, 7, 8],
    "middle": [0, 9, 10, 11, 12],
...
def normalize_landmarks(landmarks):
    """Menghitung koordinat relatif terhadap wrist dan menskalakan nilai"""
    wrist = landmarks[0]
    relative = np.array([
    ...
    return normalized.flatten().tolist()  # Menghasilkan 63 nilai fitur

def calculate_angle(p1, p2, p3):
    """Menghitung sudut derajat pada sendi tengah p2"""
...
def calculate_joint_angles(landmarks):
    """Mengekstrak 15 sudut sendi jari tangan"""
    angles = []
    ...
    return angles  # Menghasilkan 15 nilai sudut

def extract_hand_features(landmarks):
    """Mengekstrak 78 fitur dari satu tangan"""
...
def build_feature_vector(results):
    ...
    return left_features + right_features  # 156 fitur gabungan
    ...
    for letter in tqdm(letters, desc="Memproses kelas"):
        ...
        for filename in os.listdir(folder):
            ...
            if not results.multi_hand_landmarks:
                continue  # Lewati jika gambar tidak mendeteksi adanya tangan
