...
import { registerWithEmail, loginWithGoogle, onAuthChange } from "@/lib/firebase/auth";
      ...
      <div className="w-full max-w-md p-8 mx-4 z-10 backdrop-blur-md bg-slate-900/60 border border-slate-800/80 rounded-3xl shadow-2xl relative">
        <div className="text-center mb-8">
          ...
          <p className="text-slate-400 text-sm mt-2">
            Mulai belajar dan deteksi bahasa isyarat
          </p>
        </div>
        ...
        <form onSubmit={handleRegister} className="space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-3 rounded-xl bg-slate-950/60 border border-slate-800 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none text-sm transition-all placeholder:text-slate-650"
          ...
          <button
            type="submit"
            className="w-full py-3 mt-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 active:bg-indigo-700 text-white font-semibold text-sm shadow-lg shadow-indigo-600/25 transition-all flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50"
            disabled={loading || googleLoading}
          >
            {loading ? (
